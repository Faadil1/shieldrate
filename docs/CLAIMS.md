...
| One standard is fixed per employer + job scope | `jobRequests` guard in `registerWorkRequest` | SOURCE_VERIFIED |
| Cancellation cannot silently replace the standard | `cancelledWorkRequests`; `jobRequests` remains occupied | SOURCE_VERIFIED |
...
| Request expiry is enforced on-chain | `blockTimeLt` | SOURCE_VERIFIED |
| Future credential issuance is rejected on-chain | `blockTimeGte(issuedAtEpoch)` | SOURCE_VERIFIED |
| Provider removal cannot reset revocation history | removal increments and preserves `providerEpochs` | SOURCE_VERIFIED |
...
| Current npm dependency remediation reports zero vulnerabilities | Vite 8.3.0 / Vitest 5.0.1 remediation run, `npm audit` | SOURCE_VERIFIED |
