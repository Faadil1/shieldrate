...
const encode = <T>(value: T): string =>
  JSON.stringify(value, (_key, item) => {
    if (typeof item === "bigint") return { __shieldrateBigInt: item.toString() };
    if (item instanceof Uint8Array) {
      return { __shieldrateBytes: Array.from(item) };
    }
    return item;
  });
...
const decode = <T>(value: string): T =>
  JSON.parse(value, (_key, item) => {
    if (item && typeof item === "object" && "__shieldrateBigInt" in item) {
      return BigInt(item.__shieldrateBigInt as string);
    }
    if (item && typeof item === "object" && "__shieldrateBytes" in item) {
      return Uint8Array.from(item.__shieldrateBytes as number[]);
    }
    return item;
  }) as T;
  ...
  return {
    ...
    exportPrivateStates(_options?: ExportPrivateStatesOptions): Promise<PrivateStateExport> {
      ...
      const states = Object.fromEntries(
        Array.from(getScopedStates(address).entries()).map(([key, state]) => [key, encode(state)]),
      );
      return Promise.resolve({
        format: "midnight-private-state-export",
        encryptedPayload: encode({ contractAddress: address, states }),
        salt: "shieldrate-in-memory",
      });
    },
