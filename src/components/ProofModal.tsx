        ...
        <div className="px-8 py-6 border-b border-night-800 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] uppercase tracking-wider rounded px-2 py-1 border ${live ? "bg-rate-500/10 text-rate-300 border-rate-500/20" : "bg-amber-500/10 text-amber-300 border-amber-500/20"}`}>
          ...
          {stage === "select" && (
            <div>
              <div className="rounded-xl border border-night-700 bg-night-800/50 p-4 mb-6 text-xs text-mist-400">
                ...
                <p className="mt-3 text-mist-600">The proof is bound to this employer, job, a fresh 32-byte challenge, and an expiry window. It cannot be reused for another context.</p>
              </div>
              ...
              <button disabled={!walletConnected || busy} className={`btn-primary w-full ${!walletConnected || busy ? "opacity-40 cursor-not-allowed" : ""}`} onClick={startGeneration}>
                {walletConnected
                  ? `${live ? "Submit" : "Generate"} ${current.label} proof →`
                  : live
                    ? "Connect Lace wallet first"
                    : "Connect demo wallet first"}
              </button>
            </div>
          )}
          ...
          {stage === "generating" && (
            <div className="py-12 text-center">
              ...
              <h3 className="text-white font-semibold mt-6">
                {live ? "Proving and finalizing on Midnight" : "Validating locally"}
              </h3>
              <p className="text-sm text-mist-500 mt-2 max-w-md mx-auto">
                {live
                  ? "Requesting the issuer attestation, generating the zero-knowledge proof, balancing through Lace, submitting the transaction, and waiting for finalized network evidence."
  ...
  return (
    <div className="py-3 text-center">
      ...
      <h3 className={`text-2xl font-bold mt-6 ${result.passed ? "text-rate-500" : "text-red-400"}`}>
        {result.passed
          ? live ? "Midnight proof finalized" : "Attested proof ready"
          : "No proof published"}
      </h3>
      <p className="text-sm text-mist-500 mt-2 max-w-md mx-auto">
        {result.passed
          ? live
            ? "The issuer-attested private credential satisfied the policy and the transaction finalized on Midnight. Only the scoped receipt metadata below is shown."
      ...
      {receipt ? (
        <div className="bg-night-800/60 border border-night-700 rounded-xl mt-6 p-4 text-left max-w-lg mx-auto">
          ...
          <Row label="Network" value={receipt.network.toUpperCase()} />
          <Row label="Ledger" value={live ? "CONFIRMED" : "LOCAL ONLY · not submitted"} />
          <Row label="Issuer" value={receipt.issuerId} />
          {receipt.txHash && <Row label="Transaction" value={short(receipt.txHash)} mono />}
          {receipt.blockHeight !== undefined && <Row label="Block" value={receipt.blockHeight.toString()} />}
          {receipt.contractAddress && <Row label="Contract" value={short(receipt.contractAddress)} mono />}
        ...
        <div className="bg-red-950/20 border border-red-900/40 rounded-xl mt-6 p-4 text-left max-w-lg mx-auto text-xs text-red-200/80">
          Failed predicates and failed network submissions intentionally generate no shareable receipt and no successful ledger claim.
        </div>
      )}
